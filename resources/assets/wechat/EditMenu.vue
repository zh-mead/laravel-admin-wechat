<template>
    <div class="box box-info">
        ...
        <Form ref="menu" action="/admin/wechat/menus" method="post" :model="menu" :rules="ruleValidate"
              :label-width="100">
            <div class="box-body">
                <div class="fields-group">
                    ...
                    <FormItem label="菜单名称" prop="name">
                        <Input v-model="menu.name" name='name' placeholder="请填写菜单名称"/>
                    </FormItem>
                    <FormItem label="菜单类型" prop="type">
                        <Select v-model="menu.type" name="type">
                            <Option v-for="(name, index) in menuTypes" :value="index" :key="index">{{name}}</Option>
                        </Select>
                    </FormItem>
                    <FormItem v-if="menu.type === 'view'" label="菜单内容" prop="url">
                        <Input name="data" v-model="menu.url" type="textarea" placeholder="请填写url"/>
                    </FormItem>
                    <FormItem v-else-if="menu.type === 'view_limited'" label="选择文章" prop="data">
                        <Select v-model="menu.data" name="key">
                            <Option v-for="(value, index) in news" :value="value.media_id" :key="index">{{value.name}}
                            </Option>
                        </Select>
                        <!--<Input name="key" v-model="menu.data"  placeholder="media_id"/>-->
                    </FormItem>
                    <FormItem label="菜单内容" prop="data" v-else>
                        <Select v-model="menu.data" name="key">
                            <Option v-for="(name, index) in $global.events" :value="index" :key="index">{{name}}
                            </Option>
                        </Select>
                    </FormItem>
                </div>
...
<script>
    ...
    export default {
        ...
        props: ['menus', 'menuAutoId', 'types','news'],
        mounted() {
