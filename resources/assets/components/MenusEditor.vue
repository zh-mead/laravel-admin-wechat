<template>
    <div class="menus-editor">
        <div class="edit-area">
            ...
            <div v-if="$global.currentMenu" class="form">
                <div class="header">
                    <span>{{ $global.currentMenu.name }}</span>
                    <a href="javascript:void(0);" class="pull-right" @click="onRemoveCurrent"
                    >删除菜单</a>
                </div>

                <w-input label="菜单名称" v-model="$global.currentMenu.name" :has-error="hasError('name')"
                         :error-text="getError('name')" inline></w-input>

                <w-radio v-show="!currentHasSub" label="菜单内容" inline :has-error="hasError('type')"
                         :error-text="getError('type')" v-model="$global.currentMenu.type">
                    <!--<w-radio-item v-for="key of Object.keys(menuTypes)" :key="key" :label="menuTypes[key]"-->
                                  <!--:value="key"></w-radio-item>-->
                </w-radio>

                <!--<div class="content-wrapper" v-show="!currentHasSub">-->
                    <!--<component :has-error="hasError(this.currentContentField)"-->
                               <!--:error-text="getError(this.currentContentField)"-->
                               <!--v-if="currentContentComponent" :is="currentContentComponent"-->
            ...
            <div v-else class="choose-hint">在左侧选择菜单编辑
            </div>
        </div>
        <div class="footer-toolbar">
            <button class="btn btn-primary" @click="onSave" :disabled="!canSave"
                    :title="emptyMenus ? '至少要有一个菜单才能保存' : ''">保存
            </button>
            <!--<refresh :on-refresh="getData"></refresh>-->
...
<script>
    ...
    import WInput from './form/WCallbackInput.vue'
    import WRadio from './form/WRadio.vue'
    import WRadioItem from './form/WRadioItem.vue'
